...
def main(args):
    ...
    cluster_comparisons = []
    compareInfo = pd.read_csv(args.comparison_csv).set_index("cluster", inplace=True)
    ...
    for n, cluster in enumerate(compareInfo.index):
        ...
        selector = load_file_as_signatures(sigD[anchor], ksize=ksize, select_moltype=moltype)
        anchor_sig = next(selector)

        # iterate through comparison sigs
        compare_accs = compareInfo.at[anchor_acc, "cluster_members"]
